...
async def upload_spreadsheet(
    ...
    try:
        ...
        for sheet in workbook_data.sheets:
            ...
            # Get row data
            data[sheet.sheet_name] = _get_sheet_data(temp_path, sheet, sheet_columns)
...
def _get_column_names(sheet, file_path: str) -> list:
    ...
    try:
        ...
        # Get headers from the actual header row (not always row 1!)
        header_row = getattr(sheet, 'header_row_num', 1)
        headers = []
...
def _get_sheet_data(file_path: str, sheet, columns: list) -> list:
    ...
    try:
        ...
        # Skip header row, iterate data rows (header row might not be row 1!)
        header_row = getattr(sheet, 'header_row_num', 1)
